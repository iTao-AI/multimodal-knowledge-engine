...
adapter、automatic sync、hosted service 或 production deployment，也不证明真实用户采用或通用多模态理解。LLM
Wiki compatibility 并非由 v0.1.3 交付。

固定 synthetic acceptance 通过只读 isolated local query route 各运行一次 page query 和
timestamp query。两个精确响应都经由不可变的 exported Markdown 回到 canonical manifest 和
`mke.evidence_ref.v1` sidecar。OCR Phase 0 仍只记录有界本地 viability evidence，不是
production OCR。
