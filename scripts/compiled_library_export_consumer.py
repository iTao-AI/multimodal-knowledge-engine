...
_FINGERPRINTED_EXTRACTOR = re.compile(
    r"(?P<name>faster-whisper-v1|pdf-ocr-eval-v1):[0-9a-f]{64}\Z"
)
...
def _source_fingerprints(values: Sequence[str]) -> dict[str, str]:
    result: dict[str, str] = {}
    keys: set[str] = set()
    for value in values:
        if "=" not in value:
        ...
        if not re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)*", key) or key in keys:
            raise ValidationError
        data = _read_regular(Path(raw_path))
...
def validate(export: Path, supplied_sources: Sequence[str]) -> dict[str, object]:
    fingerprints = _source_fingerprints(supplied_sources)
    ...
    if (
        ...
        or manifest["markdown_format"] != "mke.compiled_markdown.v1"
    ):
        raise ValidationError
    ...
    if len(sources) != 2 or len(fingerprints) != 2:
        raise ValidationError
    ...
    for raw_entry in sources:
        ...
        if _SHA256.fullmatch(digest) is None or fingerprint not in fingerprints:
            raise ValidationError
    ...
    if (
        ...
        or seen_fingerprints != set(fingerprints)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = validate(args.export, args.source)
