...
| `mke eval retrieval-hybrid-rrf --protocol <protocol-lock.json> --candidate cjk-active-scan-qwen3-rrf-v1` | implemented in E3-D | Runs a comparison-only rank-only RRF protocol over current lexical runtime observations and the E3-C dense artifact; valid-negative development keeps `holdout_status=not_observed` and `runtime_promotion_status=not_evaluated`. |
| `mke eval retrieval-relevance-gate --protocol <protocol-lock.json> --candidate cjk-relevance-gate-reranker-v1` | implemented in E3-E | Runs a comparison-only deterministic relevance gate and reranker protocol; holdout is observed only after an exclusive development freeze and `runtime_promotion_status=not_evaluated` remains fixed. |
