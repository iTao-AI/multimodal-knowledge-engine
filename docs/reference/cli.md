...
`mke demo --verify` is deterministic and offline. It uses repository PDF fixtures and a temporary
SQLite workspace, then removes the temporary workspace before exit.
...
- `ingest` supports deterministic text-layer PDFs only.
...
Video, timestamp Evidence, Ask, MCP, HTTP, workspace UI, OCR, scanned PDFs, tables, page
coordinates, hosted coordination, and multi-worker runtime behavior are outside the current CLI
scope.
