...
- Pre-landing follow-up: `LocalCommandTranscriptProvider` now uses bounded pipe capture instead
  ...
  stable causes so sensitive exception text is redacted before stdout. Final review also normalizes
  process-start and pipe I/O failures to stable `VideoExtractionError` values and keeps known Ask
  and timestamp validation causes operator-visible.
- Final verification on 2026-06-18 after the bounded capture and error-contract fixes:
  - `uv run pytest -q`: `205 passed, 5 warnings`
  - `uv run ruff check .`: `All checks passed!`
  - `uv run pyright`: `0 errors, 0 warnings, 0 informations`
  - `uv build`: sdist and wheel built successfully
  - `uv run mke proof run`: `proof=product status=passed cases=8 passed=8 failed=0 duration_ms=19`
  - `uv run mke demo --verify`: `result=passed duration_ms=10`
