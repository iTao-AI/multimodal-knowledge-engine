...
two later CI failures, and one final targeted P2 were verified and remediated with TDD. Canonical
environment validation now compares the artifact
...
requires exact equality for the complete locator order before checking digests. Each observed
rank/`bm25()` score may differ from the independent replay by at most one ULP. Canonical artifacts
store a portable, quantized 15-significant-digit representation of finite scores, not the exact
replay scores, and recompute score-pair digests from that representation. A regression constructed
from a real score with two consecutive `math.nextafter` steps proves that a synchronized
rank-score, `bm25()`-score, and digest mutation at exactly two ULP is rejected. Coordinated
substantive score mutations still fail the independent replay gate. Snapshot mutation tests inject
a byte change on the source `rb` open,
...
- validation requires exact equality for the replayed complete locator order and permits each
  observed rank/`bm25()` score to differ from replay by at most one ULP before validating digests;
  canonical artifacts retain a portable, quantized 15-significant-digit score representation
  rather than exact replay scores; coordinated score-plus-digest and
  locator-plus-result-plus-score-plus-digest mutations fail, including an exact two-ULP mutation
  generated by two consecutive `math.nextafter` steps;
