...
def test_search_and_ask_snapshots_share_projection(tmp_path) -> None:
...
def test_snapshot_states_preserve_ask_refusal(tmp_path) -> None:
    engine = KnowledgeEngine(tmp_path / "mke.sqlite")
    try:
        ...
        assert empty.result.answer_status == no_active.result.answer_status == "insufficient_evidence"
    finally:
        engine.close()
